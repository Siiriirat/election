...
<br>
<br>
<form action="{{url('area')}}" method="POST" id="demo1" class="demo" style="display:none;" autocomplete="off">
{{ csrf_field() }}
<div class="container">
  <div class="card">
            ...
            <div class="col-md-5">
              <div class="form-group">
                <label>ชื่อผู้ดูแลเขต</label>
                  <select name="admin_id" id="admin_id" style="height: calc(3rem + 2px)" class="form-control">
                    <option disabled selected value=""></option>
                      @if (isset($admin_name))
                        <option value="{{$admin_id}}" selected>
                          {{$admin_name}}
                        </option>
                      @endif
                      @if (isset($alladmins))
                      @foreach($alladmins as $alladmin)
                        <option value="{{$alladmin->admin_id}}">
                          {{ $alladmin->admin_name}}
                        </option>
                      @endforeach
                      @endif
